...
class SimplePHPForm
{
	...
	* @param string $name Field name which can be referenced internally.
	...
	*/
	function add($name, $type, $data, $data_validation_flags, $text_title, $text_help, $text_error)
	{
		// Set default data.
		$data_default = $data;
		$key = 'simplephpform_'.$name;

		// Get form submission data from $_FILES if exists
		if ($type === 'file') {
			if (isset($_FILES[$key]) && $_FILES[$key]['error'] === UPLOAD_ERR_OK) {
				$data = $_FILES[$key];
				$this->state = self::STATE_VALIDATE;
			}
		}
	...
	*/
	function display($name=false)
	{
		...
		if(!is_string($name))
		{
			...
			$output .= '<form enctype="multipart/form-data" method="post" action="'.$this->url_action.'" class="simplephpform">';
		...
		if(array_key_exists($name, $this->input_list))
		{
			...
			else if($type == 'hidden') // Hidden type, for metadata, etc.
			{
				...
				$output .= '<div class="simplephpform_title">'.$target->text_title.'</div>'."\n";
				$output .= '<input type="'.$target->type.'" name="simplephpform_'.$target->name.'" />'."\n";
			}
		...
		foreach($this->input_list as $input)
		{
			...
			for($i = 0; $i < count($input->data_validation_flags); $i += 1)
				if(!empty($input->data_validation_flags[$i]))
				{
					...
					// Test: Is there a valid file, here an image
					if ($input->type === 'file') {
						$file = $value;
						...
						if (!in_array($file['type'], $allowed_mimes)) {
							$input->state = self::STATE_FAIL;
							$input->text_error = 'Type de fichier non autorisé.';
							$this->state = self::STATE_FAIL;
						} elseif ($file['size'] > $max_size) {
							$input->state = self::STATE_FAIL;
							$input->text_error = 'Fichier trop volumineux.';
							$this->state = self::STATE_FAIL;
						}
					}
